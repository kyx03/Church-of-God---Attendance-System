...
const Members: React.FC = () => {
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", `members_export_${hasSelection ? 'selected' : 'all'}_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="md:h-24 sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200 px-8 shadow-sm flex items-center shrink-0 py-4 md:py-0">
        <div className="max-w-7xl mx-auto flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 w-full">
            ...
            <div className="flex flex-wrap gap-2 w-full xl:w-auto">
                ...
                <Download className="w-4 h-4 md:w-5 md:h-5" /> <span className="hidden sm:inline">{selectedIds.size > 0 ? `Export Selected (${selectedIds.size})` : 'Export All (CSV)'}</span>
                </button>
