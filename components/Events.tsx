...
const Events: React.FC = () => {
  ...
  useEffect(() => {
    loadData();
    // Click outside handler for closing status dropdowns
      ...
      if (newStatus === 'cancelled') {
        ...
        if (reason) {
          updates.cancellationReason = reason;
            ...
            return; // Abort if cancelled without reason (optional strictness)
        }
      }
  ...
  const calculateBatchAbsentees = () => {
      ...
      // Find members who attended ANY event in this range
      ...
      // Chronic absentees are those active members NOT in that set
  ...
  // Generate QR Link to Internal Check-in Page
  const getQrLink = (eventId: string) => {
    const baseUrl = window.location.origin + window.location.pathname; // e.g. http://localhost:3000/
          ...
          <form onSubmit={handleSubmit} className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        {isCustomType ? (
                            <div className="flex gap-2">
                                <input 
                                    ...
                                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 bg-white text-slate-900 shadow-sm"
                                    value={newEvent.type}
                            ...
                            <div className="relative">
                                <select 
                                    className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-slate-50 focus:bg-white text-slate-900 shadow-sm appearance-none"
                                    value={newEvent.type}
                                    onChange={e => {
                 ...
                 <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-slate-700 flex items-center gap-2">
                            Location <span className="text-blue-500 text-xs font-normal">(Optional)</span>
                        </label>
                        <div className="relative">
                            ...
                            <input 
                                ...
                                className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-slate-50 focus:bg-white text-slate-900 shadow-sm"
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    ...
                    {canEdit ? (
                        <div className="relative status-dropdown-container">
                            <button 
                                onClick={() => setActiveStatusId(activeStatusId === event.id ? null : event.id)}
                                className={`flex items-center gap-1 text-xs font-bold uppercase tracking-wide px-3 py-1.5 rounded-full border transition-all ${activeStatusId === event.id ? 'ring-2 ring-blue-200 bg-white border-blue-200' : 'border-transparent hover:bg-white/50 hover:border-slate-200'}`}
                            >
                                ...
                                <div className="absolute right-0 top-full mt-2 w-40 bg-white rounded-xl shadow-xl border border-slate-100 p-1.5 z-20 animate-in fade-in zoom-in-95 duration-100">
                                    ...
                                    {['upcoming', 'in-progress', 'completed', 'cancelled'].map(s => (
                                        <button
                                            ...
                                            className={`w-full text-left px-3 py-2 text-xs font-bold uppercase rounded-lg flex items-center justify-between ${event.status === s ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}
                                        >
                                            {s}
                                            {event.status === s && <Check className="w-3 h-3" />}
                                        </button>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className="px-3 py-1.5 rounded-full bg-white/50 border border-slate-100">
                             {getStatusBadge(event.status, isPast)}
                        </div>
                    )}
              ...
              <div className="space-y-3 text-sm text-slate-600 flex-1">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center border border-slate-100 shadow-sm text-slate-400 shrink-0">
              ...
              <div className="mt-5 pt-4 border-t border-slate-200/50 flex items-center justify-between text-sm">
                 <button 
                    ...
                    className="flex items-center gap-2 text-slate-600 hover:text-blue-700 font-medium transition-colors group/attendees"
                 >
              ...
              <div className="mt-4 space-y-2">
                ...
                {(isPast && canEdit && event.status !== 'cancelled') && (
                  <button 
                    ...
                    className="w-full py-2.5 px-3 text-sm text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg flex items-center justify-center gap-2 transition-colors border border-amber-200 font-bold"
                  >
                    <MailWarning className="w-4 h-4" />
                    Notify Absentees
                  </button>
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="text-center space-y-4">
              ...
              <div className="p-4 bg-white border-2 border-slate-100 rounded-xl inline-block shadow-inner">
                <img 
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${getQrLink(checkInQrEvent.id)}`} 
            ...
            <div className="p-4 border-b border-slate-100 bg-slate-50/80 flex items-center justify-between">
              <div className="flex items-center gap-3">
                 {notifyState.step !== 'range' && notifyState.step !== (notifyState.mode === 'single' ? 'method' : 'range') ? (
                   <button 
                      onClick={() => setNotifyState(prev => ({ 
                          ...prev, 
                          step: prev.step === 'compose' ? 'method' : 'range' 
                      }))} 
                 ...
                 <div>
                    <h3 className="font-bold text-slate-900">Notify Absentees</h3>
                    <p className="text-xs text-slate-500">
                        {notifyState.mode === 'single' ? notifyState.event?.name : 'Batch Check'}
                    </p>
                 </div>
              </div>
              <button onClick={() => setNotifyState(prev => ({ ...prev, isOpen: false }))}>
            ...
            <div className="p-6">
              {notifyState.step === 'range' && (
                  <div className="space-y-4">
                      ...
                      <button 
                          onClick={calculateBatchAbsentees}
                          className="w-full py-3 bg-blue-900 text-white rounded-lg font-bold hover:bg-blue-800 transition-colors mt-2"
                      >
                          Identify Absentees
                      </button>
                  </div>
              ...
              {notifyState.step === 'method' && (
                <div className="space-y-4 animate-in slide-in-from-right-4">
                  <p className="text-slate-600 text-sm mb-4">
                    There are <strong className="text-slate-900">{notifyState.absenteeCount} active members</strong> who {notifyState.mode === 'single' ? 'missed this event' : 'have not attended any events in this range'}. How would you like to contact them?
                  </p>
                  
                  <button 
                    onClick={() => handleMethodSelect('email')}
                    className="w-full flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-blue-400 hover:bg-blue-50 transition-all group text-left"
                  >
                    <div className="p-3 bg-blue-100 text-blue-600 rounded-lg group-hover:bg-blue-200 transition-colors">
                      <Mail className="w-6 h-6" />
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-900 group-hover:text-blue-900">Send Email</h4>
                      <p className="text-xs text-slate-500">Best for newsletters and detailed updates.</p>
                    </div>
                  </button>

                  <button 
                     onClick={() => handleMethodSelect('sms')}
                     className="w-full flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-green-400 hover:bg-green-50 transition-all group text-left"
                  >
                    <div className="p-3 bg-green-100 text-green-600 rounded-lg group-hover:bg-green-200 transition-colors">
                      <Smartphone className="w-6 h-6" />
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-900 group-hover:text-green-900">Send SMS</h4>
                      <p className="text-xs text-slate-500">High open rates, best for short check-ins.</p>
                    </div>
                  </button>
                </div>
              )} 
                ...
                <div className="flex flex-col h-full animate-in slide-in-from-right-4">
                  <div className="mb-4">
                    <label className="block text-sm font-semibold text-slate-700 mb-1">
                      Compose {notifyState.method === 'email' ? 'Email' : 'SMS'} Message
                    </label>
                    <p className="text-xs text-slate-500 mb-3">
                      This message will be sent individually to {notifyState.absenteeCount} recipients.
                    </p>
                    <textarea 
                      className="w-full h-40 p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none resize-none text-sm leading-relaxed bg-white text-slate-900"
                      value={notifyState.message}
                      onChange={(e) => setNotifyState(prev => ({ ...prev, message: e.target.value }))}
                  ...
                  <button 
                    ...
                    className={`w-full py-3 rounded-lg font-bold text-white flex items-center justify-center gap-2 transition-all shadow-lg ${
                      notifyState.method === 'email' 
                      ? 'bg-blue-900 hover:bg-blue-800 shadow-blue-900/20' 
                      : 'bg-green-600 hover:bg-green-500 shadow-green-600/20'
                    }`}
                  >
                    <Send className="w-4 h-4" />
                    Send {notifyState.method === 'email' ? 'Emails' : 'Messages'}
                  </button>
                </div>
              )}
            </div>
